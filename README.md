...
> **正名链（2026-09-18 merge 窗注）**：TriLC→TriRLC→TriMLC（M 面本地控制器正名，2026-09-17 CEO 面授勘正「过渡期双跑」误判）；仓目录名 `TriLC`、bin/npm 名 `trilc` 为兼容面照旧。
>
...
## 立项期档案（历史口径；2026-09-18 merge 窗收纳）

> 以下为 2026-08 立项期描述（「无 daemon/零运行时」声明为立项期边界口径，已被现役开发期演进取代——8713 M 面 daemon 在役）。保留作历史叙事，不作现行口径。
